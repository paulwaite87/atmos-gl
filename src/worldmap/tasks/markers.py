...
class MarkerUpdater(Updater):
    ...
    def run(self):
        ...
        # Resolve the GFS run + the forecast hour valid "now" (same datum the layers use).
        self.get_gfs_state()
        ...
        if temp_f is None and wind_f is None and rh_f is None:
            logger.warning(
                f"Markers: no temperature/wind/humidity fields for f{fhour:03d}; "
                "skipping (data collector may not have ingested them yet)."
            )
            return
