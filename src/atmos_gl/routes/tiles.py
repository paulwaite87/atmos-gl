...
pre-rendered and published by the layer builder (see atmos_gl.tiles.raster_tiles); this
...
def layer_meta(layer: str, config: AtmosGLConfig = Depends(get_config)):
    spec = _spec(layer)
...
def layer_tile(
    layer: str, z: int, x: int, y: int, config: AtmosGLConfig = Depends(get_config)
):
    spec = _spec(layer)
