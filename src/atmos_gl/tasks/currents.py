...
class CurrentsUpdater(VectorFieldUpdater):
    ...
    def plot(self, field0, state: ForecastState):
        ...
        # Dilate 1 cell (8-connectivity, covering diagonal-only coastal corners) before
        # cutting: both consumers of this texture -- the fill shader (createFillLayer's
        # LINEAR-filtered u_tex0/u_tex1, hard alpha>=0.5 discard) and the particle
        # engine's VEL_SAMPLE (same LINEAR-tap-then-threshold pattern) -- blend alpha
        # AND colour/velocity across every texel boundary, including the true
        # coastline edge. A sample landing more than half-way toward the (alpha=255)
        # ocean side survives the discard with bleed-through colour/motion even on the
        # land side of the real coastline. Same fix, same reasoning, as SST's land
        # mask (see docs/adr/0014-dilate-sst-land-mask-for-linear-filtering-bleed.md).
