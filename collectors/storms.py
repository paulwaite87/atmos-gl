...
Pure data (no render): the data_collector scrapes the ATCF b-deck/a-deck files, builds
tracks + forecast cones, and upserts storms; the frontend reads them via the /api/storms
route. The ATCF parsing is unchanged from the former layer_builder updater.
"""
import logging
import math
...
class StormsCollector:
    ...
    def collect(self):
        ...
        for storm in active_storms:
            ...
            logger.debug(
                f"Upserted storm {sid} ({storm['name']}) into database with "
