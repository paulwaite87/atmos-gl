...
| **Flood Risk** | A land-only layer (`tasks/flood_risk.py`, issue #371) with two independently-sourced modes sharing one `flood_risk` section/output path, not two views of the same data: **Live** (`FloodRiskLiveCollector`, `collectors/flood_risk.py`) is Copernicus GloFAS ensemble river-discharge forecast classified into 2yr/5yr/20yr return-period severity bands (0-3); **Historical** (`FloodRiskHistoricalCollector`) is Copernicus/JRC's static Global River Flood Hazard map (100-year return period) mosaicked once from per-tile downloads and cached forever, classified into a fixed 0-4 depth-hazard category. The two bands are deliberately NOT comparable (different scales, different physical meaning) — each keeps its own encode domain/palette/legend, both server-side (`_LIVE_ENCODE_DOMAIN`/`_HISTORICAL_ENCODE_DOMAIN`) and client-side (`ui/modules/flood_risk.js`'s `ENCODE_DOMAIN`/`LIVE_COLORS`/`HISTORICAL_COLORS`). `FloodRiskUpdater` renders both variants every cycle regardless of the configured mode and publishes only the selected one to the canonical output path (the same "render everything, publish only what's selected" shape `GhgUpdater` established for species/mode) — so a mode switch applies on the frontend's next poll tick, no render wait, and the GLOFAS_API_KEY gate (Live-mode-only; Historical needs no credential) never disables the whole section, only Live. | flood layer, GloFAS layer |
