#!/usr/bin/env bash
# Atmos GL control script -- start/stop/monitor the pre-built Docker Compose stack.
...
cmd_status() {
    [ -f .env ] || die "No .env found -- run this from your atmos-gl install directory."
...
cmd_logs() {
    if [ "${1:-}" = "save" ]; then
        local outfile="atmos-gl-logs-$(date +%Y%m%d-%H%M%S).txt"
