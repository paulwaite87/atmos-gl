...
// flow_coherence_radius is live-tunable). See _particlegl_primitives.js's COH_H_FS/COH_V_FS
...
uniform sampler2D u_wind;
uniform sampler2D u_cmap;
uniform float u_res, u_vmax, u_maxspeed, u_pointSize, u_calmFade, u_alpha;
out vec4 v_color;
const float WV_PI = 3.141592653589793;
const float WV_LATMAX = 1.4844222297453324;
#ifdef POS_FLOAT
...
export function createParticleGLController(map, opts) {
    ...
    // ---- GL plumbing (compile/linkProg/makeTex/makeStateTex/randomAge shared with
    // _currentparticles_gl.js via _particlegl_primitives.js; randomState stays here --
    ...
    const buildState = (gl) => {
        for (let i = 0; i < 2; i++) {
            ...
            stateTex[i] = makeStateTex(gl, RES, RES, randomState(), floatPos);
            ageTex[i]   = makeTex(gl, RES, RES, randomAge(RES), gl.NEAREST);
    ...
    const getStreakProg = (gl, shaderData) => {
        ...
        const prog = linkProg(gl, vs, DRAW_FS, floatPos, sectionKey);
    ...
    const getPointProg = (gl, shaderData) => {
        ...
        const vs = `#version 300 es\n${shaderData.vertexShaderPrelude}\n${shaderData.define}\n${POINT_VS_BODY}`;
        const prog = linkProg(gl, vs, POINT_FS, floatPos, sectionKey);
    ...
    const makeLayer = (cfg) => ({
        ...
        onAdd(m, gl) {
            ...
            quadProg = linkProg(gl, SCREEN_QUAD_VS, SCREEN_FS, floatPos, sectionKey);
            if (quadProg) {
            ...
            blendProg = linkProg(gl, SCREEN_QUAD_VS, BLEND_FS, floatPos, sectionKey);
            // Separable direction-coherence programs (horizontal then vertical).
            cohHProg = linkProg(gl, SCREEN_QUAD_VS, COH_H_FS, floatPos, sectionKey);
            cohVProg = linkProg(gl, SCREEN_QUAD_VS, COH_V_FS, floatPos, sectionKey);
