/**
 * Temperature Module - Temperature (absolute or anomaly) map with automatic background updates
 */
export function loadLayer(map, config) {
    ...
    const sourceId = 'temperature-source';
    ...
    if (legendStack) {
        ...
        const existingSlot = document.getElementById('temperature-legend-slot');
        if (existingSlot) existingSlot.remove();

        const slot = document.createElement('div');
        slot.id = 'temperature-legend-slot';
